...
def guess_file_type(path):
    ...
    if len(line) < (len(path) + 2):
        print >>sys.stderr, "Malformed output from 'file %s': '%s'" % \
            (path, line)
        exit(1)
    ...
    for attempt in [5, 4, 3, 2, 1, 0]:
        if os.path.exists(destination):
            if looks_like_an_archive(destination):
                print >>sys.stderr, \
                    "%s exists and looks like an archive" % destination
                # Refresh the archive's last access time.
                # If the spec file which depends on this archive has been
                # modified, it will have a later access time than the archive
                # and make will continually rebuild it.   'Touch' the archive,
                # to prevent this happening.   There is a potential race here:
                ...
                print >>sys.stderr, \
                    "%s is not an archive, deleting it" % destination
                os.unlink(destination)
                if attempt == 0:
                    print >>sys.stderr, \
                        "That was our last attempt so giving up."
                    exit(1)
        else:
...
def main():
    if len(sys.argv) != 3:
        print >>sys.stderr, \
            "Wrong number of arguments. Use %s <url> <destination>" % \
            sys.argv[0]
        exit(1)
    ...
    if url.netloc == "github.com" and path[-3] == "archive":
        ...
        if not ext:
            print >>sys.stderr, \
                "I did not recognise extension of %s. I know about: %s" % \
                (url.path, ", ".join(possible_exts))
            exit(1)
        url_path = "/".join(path[0:-2] + [path[-2] + ext])
        url_string = str(urlparse.urlunsplit(
            (url.scheme, url.netloc, url_path, url.query, url.fragment),))
