...
def parse_args_or_exit(argv=None):
    ...
    if parsed_args.link:
        ...
        if link.schema_version == 1:
            ...
            parsed_args.patchdata['patchequeues'] = []
            for patchqueue in link.patchqueue_sources:
                target = [arg for arg in argv if
                          arg.endswith('%s.tar' % (patchqueue))]
                if target:
                    parsed_args.patchdata['patchequeues'].append(target[0])
...
def extract_v2_patches(tmpdir, spec, tmp_specfile, link, patchdata):
    ...
    if 'patchqueues' in patchdata:
        ...
        for patchqueue in link.patch_queues:
            tarname = '%s.tar' % patchqueue
            patchset = [patchqueue for patchqueue in patchqueues
                        if os.path.basename(patchqueue) == tarname][0]
            with Patchqueue(patchset,
                            branch=link.
                            patch_queues[patchqueue]['patchqueue']) as patches:
